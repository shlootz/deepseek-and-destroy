...
"""))
    ...
    def test_dsd_attempt_minimum_hop_flow_implementer_to_reviewer_direct_accept(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            state={"project_worktree":str(project.resolve()),"execution_status":"active","next_action":"launch implementer","worker_rules":json.loads(prep.stdout),"worker_runtime":{"harness":"opencode-cli","model":"deepseek/deepseek-flash","opencode":{"run_db":str((root/"external"/"workers.db").resolve())}},"phases":{"p1":{"status":"in-progress","tasks":{"U1":{"status":"prepared","current_contract":{"revision":1,"path":str(contract.resolve()),"sha256":hashlib.sha256(contract.read_bytes()).hexdigest()}}}}}}
